...
export class RecordsService {
  ...
  * 4. Returns the full list of records that are part of these duplicate groups.*/
  ...
  async mergeDuplicateGroup(primaryId: string, duplicateIds: string[]) {
    ...
    const primaryAdj = new Set((primary.adjNumber || []).map((a: any) => String(a?.value || '').trim()));

    for (const dup of duplicates) {
      if (norm(dup.provider) !== providerNorm || norm(dup.ptName) !== ptNorm) {
        throw new BadRequestException('Selected records do not match the same Provider and Patient Name.');
      }
      const dupAdj = new Set((dup.adjNumber || []).map((a: any) => String(a?.value || '').trim()));
      const hasCommonAdj = [...dupAdj].some((v) => primaryAdj.has(v));
      if (!hasCommonAdj) {
        throw new BadRequestException('Selected records do not share a common ADJ Number with the primary record.');
      }
    }
    ...
    const snapshot = (r: any) => ({
      ...
      assignedCollector: r.assignedCollector
        ? {
            _id: r.assignedCollector?._id ? String(r.assignedCollector._id) : String(r.assignedCollector),
    ...
    if (commentsToInsert.length === 0) {
      ...
      const delRes = await this.recordModel.deleteMany({ _id: { $in: cleanDupIds.map((id) => new Types.ObjectId(id)) } });
      return {
        primaryId,
  ...
  async mergeSelectedDuplicates(primaryId: string, duplicateIds: string[]) {
    ...
    const docs = await this.recordModel
      .find({ _id: { $in: allIds } })
      .select({ comments: 1, provider: 1, ptName: 1, adjNumber: 1, assignedCollector: 1, recordCreatedAt: 1, createdAt: 1 })
      .populate('assignedCollector', 'username')
      .lean()
      .exec();
    ...
    if (duplicates.length !== duplicateIds.length) {
      ...
      throw new BadRequestException(`Some duplicate records were not found: ${missing.join(', ')}`);
    }
    ...
    const existingMergedKeys = new Set(
      (primary.comments || [])
        .filter((c: any) => c?.isFromMergedRecord && c?.sourceRecordId && c?.sourceCommentId)
        .map((c: any) => `${String(c.sourceRecordId)}:${String(c.sourceCommentId)}`),
    );
    ...
    for (const dup of duplicates) {
      ...
      for (const c of dup.comments || []) {
        ...
        const author = (c as any).author && typeof (c as any).author === 'object' && (c as any).author._id
          ? (c as any).author._id
          : (c as any).author;
    ...
    const deleteRes = await this.recordModel.deleteMany({ _id: { $in: duplicateIds.map((id) => new Types.ObjectId(id)) } });
